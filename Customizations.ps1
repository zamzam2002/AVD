...
ChatGPT said:
Here’s a modified version of the original script that installs only English (Australia) without requiring parameters, making it fully self-contained and ready to run directly:
...
<# 
    ...
    Purpose: Install English (Australia) Language Pack without requiring parameters
#>
...
function Install-LanguagePack {
    ...
    END {
        if (Test-Path -Path $templateFilePathFolder) {
...
# Call the function directly without parameters
